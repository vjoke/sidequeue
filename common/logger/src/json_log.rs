...
impl JsonLogEntry {
    pub fn new(name: &'static str, json: json::Value) -> Self {
        let timestamp = SystemTime::now()
            ...
            .as_millis()
            .try_into()
            .expect("Unable to convert u128 to u64");
        JsonLogEntry {
            name: name.into(),
            timestamp,
            json,
...
///
/// Note that this method acquires global lock for breif moment.
/// This means that very hot threads can not use this method concurrently, otherwise they
/// will content for the same lock.
