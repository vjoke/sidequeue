...
fn publish_metrics_test() {
    let counter_opts = Opts::new("sidequeue_test_counter", "sidequeue test counter help");
    ...
    assert_eq!("sidequeue test counter help", m.get_help());
    assert_eq!("sidequeue_test_counter", m.get_name());
